...
namespace ADO.NETHelper
{
    public class OracleHelper2
    {
        ...
        private static int GetMaxID(string fieldName, string tableName)
        {
            ...
            object obj = OracleHelper2.GetSingle(strsql);
            if (obj == null)
            {
                return 1;
            }
        ...
        {
            object obj = OracleHelper2.GetSingle(strSql, cmdParms);
            int cmdresult;
        ...
        public static T ExecuteEntity<T>(string cmdText, CommandType commandType, params OracleParameter[] parms)
        {
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(cmdText, connection))
                {
                    ...
                    while (reader.Read())
                    {
                        obj = OracleHelper2.ExecuteDataReader<T>(reader);
                    }
                }
            }
            return obj;
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(cmdText, connection))
                {
                    try
                    {
                        ...
                        while (reader.Read())
                        {
                            T obj = OracleHelper2.ExecuteDataReader<T>(reader);
                            list.Add(obj);
                        }
                    }
                    catch (Exception e)
